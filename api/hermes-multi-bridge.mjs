...
function buildPrompt(entry, packet) {
  ...
  const toolBlock = [
    ...
    `  {"type":"share_files","conversation_id":"<id>","body_md":"<optional one-liner, can be empty>","reply_to":"<optional>","files":[{"url":"https://…","name":"optional.jpg"},{"url":"…"}]}`,
    `                                                                — server-side fetches each URL and posts them as attachments in the conversation. Use this whenever you'd otherwise shell out to urllib/curl + /agent-api/uploads. Up to 10 files, 20MB each.`,
