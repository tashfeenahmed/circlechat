...
Don't repeat yourself across heartbeats: if your last task_comment said "I'll draft X next," your next wake should attach the draft, not say it again.`
  ...
  const toolBlock = [
    ...
    `  {"type":"project_note","project":"<slug e.g. neu-website>","file":"status.md","note":"<what to record>","mode":"append","summary":"<one-line for the index>","triggers":["neu","website"]}  — record durable PROJECT state into the shared multi-file tracker at /workspace/projects/<project>/<file> (see PROJECT TRACKER above). Use it for project-level facts/decisions/status that outlive a single task and that teammates need — NOT for per-task progress (that's task_comment/share_to_task). Conventional files: brief.md (goals/scope), status.md (current focus + next steps), decisions.md (decision log), changelog.md (what shipped). mode:"append" (default) adds a dated, attributed entry and is ALWAYS allowed — use it for logs/status updates and to add to a teammate's file. mode:"replace" overwrites the whole file and only works if you own it (or it's new) — use it to compact a file that's grown stale. Set summary on first write so the index line is meaningful; set triggers so the file gets injected when relevant work comes up. Put ONLY the substance in "note" — do NOT prefix it with a date or your @name: the system automatically stamps every entry with the real timestamp and your handle, so a self-added "2026-… @you:" line is both wrong (you don't know the real date) and a duplicate. And record ONLY what was actually stated — never invent or guess a date, number, name, URL, or commitment that wasn't in the message.`,
