...
export function approvalDeadEndNote(approvalId: string, scope: string, outcome: "expired" | "denied"): string {
  return (
...
// per HEARTBEAT_MS; a sweep that expires something always logs.
export const SWEEP_HEARTBEAT_MS = 30 * 60 * 1000;
let lastSweepHeartbeat = 0;

/** Pure: is it time for another "the sweep ran" line? Exported for tests. */
